...
function applyHistory(){const list=document.querySelector('.workspace-timeline-list');if(!list)return;const events=[...list.querySelectorAll('.workspace-event')];const old=list.querySelector('.atlas-history-lock');old?.remove();events.forEach(el=>{el.hidden=false;});if(isPro()||events.length<=5)return;events.slice(5).forEach(el=>{el.hidden=true;});const c=copy(),lock=document.createElement('div');lock.className='atlas-history-lock';lock.innerHTML=`<strong>Atlas Pro</strong><span>${c.historyLocked}</span><button type="button">${c.historyCta}</button>`;lock.querySelector('button').addEventListener('click',()=>window.dispatchEvent(new CustomEvent('atlas:upgrade-intent',{detail:{source:'history_limit'}})));list.appendChild(lock);}
function applyPdf(){const button=document.getElementById('pdf-button');if(!button)return;const c=copy();button.textContent=isPro()?c.pdfPro:c.pdfFree;let note=document.querySelector('.atlas-pdf-tier-note');if(isPro()){note?.remove();return;}if(!note){note=document.createElement('span');note.className='atlas-pdf-tier-note';button.insertAdjacentElement('afterend',note);}note.textContent=c.pdfHint;}
function refresh(){requestAnimationFrame(()=>{applyHistory();applyPdf();});setTimeout(()=>{applyHistory();applyPdf();},100);}
export function initHistoryPdfGating(){if(initialized)return;ensureStyles();const observer=new MutationObserver(refresh);observer.observe(document.body,{subtree:true,childList:true});window.addEventListener('atlas:entitlements',refresh);window.addEventListener('atlas:locale',refresh);refresh();initialized=true;}
