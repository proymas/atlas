...
export function renderReport(report = {}) {
  ...
  byId('verdict').textContent = safeText(report.verdict) || 'VALIDAR ANTES DE CONSTRUIR';
  byId('summary').textContent = safeText(report.executiveSummary || report.summary) || 'Atlas ha identificado qué deberías demostrar antes de invertir más tiempo.';
  ...
  byId('experiment-name').textContent = safeText(experiment.name) || 'Experimento de 7 días';
  renderList('experiment-steps', experiment.steps);
}

export async function revealReport(report) {
  setProgress(96, 'Informe preparado');
