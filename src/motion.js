...
const SURFACE_SELECTOR = '[role="dialog"],.modal,.overlay,.drawer,.sheet,.popover,.panel,.copilot-modal,.compare-modal,.experiment-modal,.upgrade-modal';
...
function installMotionStyles(){
  ...
  style.textContent=`
    :root{--atlas-ease:cubic-bezier(.22,.8,.24,1);--atlas-fast:135ms;--atlas-medium:200ms}

    ${BUTTON_SELECTOR}{
      transition:border-color var(--atlas-fast) ease,background-color var(--atlas-fast) ease,box-shadow var(--atlas-fast) var(--atlas-ease),opacity var(--atlas-fast) ease!important;
      -webkit-tap-highlight-color:transparent;
    }
    @media(hover:hover) and (pointer:fine){
      ${BUTTON_SELECTOR}:not(:disabled):hover{box-shadow:0 6px 18px rgba(30,112,190,.12)}
      .workspace-open:not(:disabled):hover{box-shadow:0 0 0 1px rgba(91,201,255,.18),0 6px 18px rgba(30,112,190,.10)}
    }
    ${BUTTON_SELECTOR}:focus-visible{outline:2px solid rgba(95,205,255,.8)!important;outline-offset:3px!important}

    .atlas-motion-enter{animation:atlasSurfaceIn var(--atlas-medium) var(--atlas-ease) both;will-change:transform,opacity}
    @keyframes atlasSurfaceIn{from{opacity:0;transform:translateY(7px)}to{opacity:1;transform:translateY(0)}}
    ...
    @media(max-width:700px){.atlas-motion-enter{animation-duration:165ms}.atlas-intro::before{display:none}.atlas-intro-logo{text-shadow:none}}
    @media(prefers-reduced-motion:reduce){
      *,*::before,*::after{scroll-behavior:auto!important}
      ...
      ${BUTTON_SELECTOR}{transition:none!important}
    }
  `;
  document.head.appendChild(style);
}

function looksLikeSurface(node){return node instanceof HTMLElement && node.matches?.(SURFACE_SELECTOR);}
function animateSurface(node){
  ...
  node.addEventListener('animationend',()=>{node.classList.remove('atlas-motion-enter');node.style.willChange='auto';},{once:true});
  return true;
}

function animateButtonPress(target){
  if(reducedMotion() || !(target instanceof HTMLElement) || target.matches(':disabled')) return;
  const isWorkspace=target.classList.contains('workspace-open');
  target.getAnimations?.().filter(animation=>animation.id==='atlas-button-press').forEach(animation=>animation.cancel());
  const frames=isWorkspace
    ? [{opacity:1,boxShadow:'0 0 0 0 rgba(91,201,255,0)'},{opacity:.78,boxShadow:'0 0 0 3px rgba(91,201,255,.16)'},{opacity:1,boxShadow:'0 0 0 0 rgba(91,201,255,0)'}]
    : [{opacity:1,transform:'scale(1)'},{opacity:.82,transform:'scale(.975)'},{opacity:1,transform:'scale(1)'}];
  const animation=target.animate(frames,{duration:isWorkspace?150:165,easing:'cubic-bezier(.22,.8,.24,1)'});
...
function animateWorkspaceOpen(root){
  if(reducedMotion() || !(root instanceof HTMLElement) || root.classList.contains('hidden')) return;
  const panel=root.querySelector('.workspace-panel');
  if(!(panel instanceof HTMLElement)) return;
  panel.getAnimations?.().forEach(animation=>{if(animation.id==='atlas-workspace-open')animation.cancel();});
  const animation=panel.animate(
    [{opacity:.86,transform:'translateY(8px)'},{opacity:1,transform:'translateY(0)'}],
    {duration:185,easing:'cubic-bezier(.22,.8,.24,1)',fill:'none'}
  );
  animation.id='atlas-workspace-open';
}

let workspaceObserver=null;
function attachWorkspaceObserver(){
  const root=document.getElementById('workspace-modal');
  if(!root || workspaceObserver) return Boolean(root);
  workspaceObserver=new MutationObserver(records=>{
    if(records.some(record=>record.attributeName==='class')) animateWorkspaceOpen(root);
  });
  workspaceObserver.observe(root,{attributes:true,attributeFilter:['class']});
  return true;
}
  ...
  observer=new MutationObserver(records=>{
    for(const record of records){
      for(const node of record.addedNodes){
        ...
        if(node.id==='workspace-modal') attachWorkspaceObserver();
...
function observeDynamicUi(){startObserver();document.addEventListener('visibilitychange',()=>document.hidden?stopObserver():startObserver());}
...
function runIntro(){
  ...
  const leave=()=>{intro.classList.add('is-leaving');document.documentElement.style.overflow=previousOverflow;setTimeout(()=>intro.remove(),300);};
  setTimeout(leave,820);
}

function init(){installMotionStyles();bindButtonMotion();observeDynamicUi();runIntro();}
